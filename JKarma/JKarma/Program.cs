...
namespace JKarma
{
    class Program
    {
        private static string Author = "Husmeador12";
        private static string AddonName = "JKarma";
        ...
        {
            if (myHero.Hero != Champion.Karma) { return; } //<font color = \"#0099FF\">Saamble<font color = \"#FFFFCC\"> is loaded. ^.^</font>
            ...
            Menu.AddLabel("Credits to iCreative :3");
            ...
            SubMenu["Misc"].Add("Gapclose", new CheckBox("Use W on gapclose spells", true));
            SubMenu["Misc"].Add("AutoProtect", new CheckBox("Use E to protect allies", true));
            SubMenu["Misc"].Add("Channeling", new CheckBox("Use W on channeling spells", true));
        ...
        {
            foreach (Obj_AI_Base minion in ObjectManager.Get<Obj_AI_Minion>().Where(f => f.IsValidTarget(1000f)))
            //foreach (Obj_AI_Base minion in EntityManager.MinionsAndMonsters.EnemyMinions(EntityManager.UnitTeam.Enemy, myHero.Position.To2D(), 1000f))
            {
        ...
        static void JungleClear()
        {
            ...
            {
                ///foreach (Obj_AI_Base minion in EntityManager.MinionsAndMonsters.Monsters(myHero.Position.To2D(), 1000f))
                foreach (Obj_AI_Base minion in ObjectManager.Get<Obj_AI_Minion>().Where(f => f.IsValidTarget(1000f)))
                {
        ...
        static void OnCreateObj(EloBuddy.GameObject sender, EventArgs args)
        {
            if (sender.Name.ToLower().Contains("missile"))
            {
                ...
                if (missile.SpellCaster.IsMe)
                {
                    ...
                    if (name.Contains("karmafiremissile"))
                    {
                        _Q["Object"] = sender;
                        _Q["IsReturning"] = false;
                    }
                    else if (name.Contains("karmaresolutionhit"))
                    {
                        _E["Object"] = sender;
                    }
                }
        ...
        static void OnDeleteObj(GameObject sender, EventArgs args)
        {
            if (sender.Name.ToLower().Contains("missile"))
            {
                ...
                {
                    var name = missile.SData.Name.ToLower();
                    if (name.Contains("ahriseducemissile"))
                    {
                        _E["Object"] = null;
                    }
                }
        ...
        static void OnApplyBuff(Obj_AI_Base sender, Obj_AI_BaseBuffGainEventArgs args)
        {
            ...
            {
                var buff = args.Buff;
                if (buff.Name.ToLower() == "karmamantra")
                {
